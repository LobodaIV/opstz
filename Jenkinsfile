    ...
    try {
	    ...
	    if(!runRes) {
	 	sh(returnStatus: true, script: "docker-machine create --driver amazonec2 --amazonec2-open-port 80 --amazonec2-region eu-west-1 --amazonec2-zone 'c' aws-nginx-instance")
	        sh 'eval $(docker-machine env aws-nginx-instance --shell bash); docker build --pull -t libroli1988/opstz:latest https://github.com/LobodaIV/opstz.git; docker run -d --name nginx-lua -p 80:80 libroli1988/opstz'
	    } else {
